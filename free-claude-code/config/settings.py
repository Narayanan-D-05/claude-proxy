...
class Settings(BaseSettings):
    ...
    enable_model_identity_cleanup: bool = Field(
        default=False, validation_alias="ENABLE_MODEL_IDENTITY_CLEANUP"
    )
    ...
    def resolve_model(self, claude_model_name: str) -> str:
        ...
        1. If it contains a '/', it's already resolved.
        2. Check for an alias in environment variables (MODEL_ALIAS_<NAME>).
        3. Match specific Claude models (opus/sonnet/haiku).
        4. Fall back to the default MODEL.
        """
        if "/" in claude_model_name:
            return claude_model_name

        # Check for environment-defined alias (e.g. MODEL_ALIAS_DEEPSEEK)
        alias_key = f"MODEL_ALIAS_{claude_model_name.upper().replace('-', '_')}"
        if alias_val := os.environ.get(alias_key):
            return alias_val
