...
def build_request_body(request_data: Any, nim: NimSettings) -> dict:
    ...
    # Determine if this is a "Thinking/Reasoning" model that supports the reasoning_budget parameter
    # Currently known to work with: Kimi, Nemotron-4-340B-Instruct, Nebula, DeepSeek-R1
    model_id = body.get("model", "")
    model_name_only = model_id.split("/")[-1] if "/" in model_id else model_id
    is_thinking_model = any(
        k in model_id.lower() for k in ["kimi-k2-thinking", "nemotron", "nebula", "deepseek-r1", "reasoning"]
    )
    
    # Only apply reasoning_budget if the model is explicitly detected as a thinking model
